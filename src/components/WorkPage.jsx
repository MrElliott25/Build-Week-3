...
import { App, ArrowRight } from "react-bootstrap-icons";
import Carousel from "react-bootstrap/Carousel";
import "./WorkPage.css";
import { workPageFetch } from "../utils/fetch";
import { ArrowLeftCircleFill, ArrowRightCircleFill, X } from "react-bootstrap-icons";
import { AppContext } from "../assets/contexts/context";

let workAPI = "https://strive-benchmark.herokuapp.com/api/jobs";

const WorkPage = function () {
  const [categoria, setCategoria] = useState("candidatura semplice");
  const { generalOutput } = useContext(AppContext);
  const { selectionObject } = useContext(AppContext);
  const { setPopulateDropdown } = useContext(AppContext);

  const { showJob, setShowJob } = useContext(AppContext);
  const { searchList, setSearchList } = useContext(AppContext);

  const [data, setData] = useState({});

  const hasInitializedJobs = useRef(false);

  const loadData = async () => {
    const data = await workPageFetch(workAPI);
    setData(data);
    if (!hasInitializedJobs.current && data) {
      const filteredCarouselList = data.data.slice(0, 6);
      setShowJob(await filteredCarouselList);
      console.log(filteredCarouselList);
      hasInitializedJobs.current = true;
    }
  };
  ...
  const jobOptions = () => {
    let filteredList = [];
    let searchListToPush = [];
    setSearchList("");
    data.data.slice(0, 5).map((element) => {
      filteredList.push(element.title);
      searchListToPush.push(element);

      console.log(searchList);
    });
    setSearchList(searchListToPush);
    setPopulateDropdown(filteredList);
  ...
  useEffect(() => {
    if (generalOutput) {
      workAPI = "https://strive-benchmark.herokuapp.com/api/jobs" + "?search=" + selectionObject;
      jobOptions();
    } else {
      workAPI = "https://strive-benchmark.herokuapp.com/api/jobs";
    }

    console.log(workAPI);
   loadData();
  }, [selectionObject, generalOutput]);

  //per il carosello
  const { slideCarosello, setSlideCarosello } = useContext(AppContext);
        ...
        <Container>
          ...
          <div className="mb-3 ">
            ...
            <p className=" m-0 mb-4 text-secondary">
              in base al tuo profilo,alle tue preferenza e ad attività come candidature, richieste e salvataggi
            </p>
            {searchList.length > 0
              ? searchList.slice(0, 5).map((res) => (
          ...
          <div className="mb-3">
            <Row>
              <div className="d-flex justify-content-between">
                <Col xs={6}>
                  <div>
                    <h3 className="m-0">Esplora le aziende che assumono per le tue competenze</h3>
              ...
              >
                <Carousel.Item className="text-center">
                  ...
                  <div className="d-flex">
                    {showJob.slice(0, 2).map((res) => (
                      <a href={res.url} key={res._id}>
                        {" "}
                        <div className=" border border-dark-subtle p-3 mx-2 my-2">
                          <h5>{res.title}</h5>
                          <p>
                            {res.company_name} • {res.candidate_required_location} • {res.salary}
                          </p>
                          <section>
                            <div
            ...
            <div>
              ...
              {categoria === "altro" && (
                <div className="per-bottoni">
                  ...
                  <Row>
                    <Col xs={12} sm={4}>
                      <button type="button" className="py-3 glow-btn w-100 text-start ps-4 m-1 bg-bianco rounded">
                        <img
